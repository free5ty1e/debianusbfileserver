...
for i in "${!RTSP_STREAM_URLS[@]}"; do 
	...
	sudo ffmpeg -loglevel fatal -attempt_recovery 1 -recovery_wait_time 1 -i "$STREAM_URL" -vf fps=fps=$CAPTURE_FPS -update 1 -an -y -s $CAPTURE_RESOLUTION "$CAPTURE_LOCATION/$STREAM_TITLE.jpg" </dev/null &
	#-stimeout $RTSP_TIMEOUT -drop_pkts_on_overflow 1
