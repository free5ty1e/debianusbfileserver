...
usage_notes() {
	echo "USAGE: "
    echo "syncdrive.sh /media/VolumeOne /media/VolumeOneBak nodryrun excludedPattern"
    echo "(nodryrun is optional and in fact can be most any word)"
    echo "(excludedPattern is optional and only available if nodryrun is present)"
}
...
if [ -z "$4" ] ; then 
	echo "Exclude disabled"
	SYNCEXCLUDEPARAM=""
	SYNCEXCLUDEOPTION=""
else
	echo "Excluding $4"
	SYNCEXCLUDEPARAM="--exclude"	
	SYNCEXCLUDEOPTION="$4"
fi
